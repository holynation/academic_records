...
class QueryHtmlTableObjModel extends Model
{
	...
	*/
	...
	private function htmlDropDownAction(){
		$result = "<button type='button' class='btn p-0 dropdown-toggle hide-arrow' data-bs-toggle='dropdown'>
	...
	*/
	private function addTableAction(array $action,array $data=null){
		...
		foreach ($action as $key => $value) {
			...
			if($this->classname == ('owners' || 'hirers') && $label == 'edit'){
				$label = "update profile";
			}
			$result.= "<span data-ajax='$ajax' data-critical='$critical' $editClass ><a class='dropdown-item text-center text-capitalize'  href='$link'>$label</a></span>";
		}
